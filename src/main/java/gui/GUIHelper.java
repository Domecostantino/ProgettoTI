...
public class GUIHelper {
	...
	private boolean successful_connection = false;
	...
	private GUIHelper() {
		successful_connection = db.inizializzaDB();
	...
	void showMeanRecoveryRatePerChannelModel() {
		...
		JFreeChart chart = ChartFactory.createBarChart3D(
				"Recovery rate medio per modello di canale\nCod di canale: " + NAMES_MAP.get(chan_cod.toString()), "",
				"Rate", dataset);
	...
	void showCompressionRatePerFile() {
		...
		JFreeChart chart = ChartFactory.createBarChart3D(
				"Compression rate per file\n" + "Cod di sorgente: " + NAMES_MAP.get(source_cod.toString()), "", "Rate",
				dataset);
	...
	void showDelayMeans() {
		...
		JFreeChart chart = ChartFactory.createPieChart3D(
				"Tempi degli step di simulazione\nCod di sorgente: " + NAMES_MAP.get(source_cod.toString())
						+ ", Cod di canale: " + NAMES_MAP.get(chan_cod.toString()), // chart title
				dataset, // data
				true, // include legend
				true, false);
	...
	void showErrorComparison() {
		...
		GestioneDB.CHAN_COD chan_cod = db.getChanCodeSim(simulation.getChannelCoder().getClass().getName(), simulation);
		JFreeChart chart = ChartFactory.createBarChart3D("Codifica di canale: " + NAMES_MAP.get(chan_cod.toString()),
				"", "%", dataset);
	...
	public void run() {
		...
		Thread highlight = new Thread(() -> {
			...
			highlight(GUI.getInstance().getSourceText(), findDifferences(outString, inString),
					new Color(0f, 0.7f, 0.3f, 0.2f));
		});
		highlight.start();
	}
